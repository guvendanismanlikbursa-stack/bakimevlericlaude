...
class OpsController extends Controller
{
     ...
     * hatasi icin - bu paylasimli hosting hesabinin GERCEK MySQL baglanti
     * limitini ve o an kac baglanti kullanildigini/en fazla kac baglanti
     * kullanildigini dogrudan gosterir.
     */
    private function mysqlConnectionsDiagnose(): string
    {
        $maxConn = DB::select("show variables like 'max_connections'");
        $maxUserConn = DB::select("show variables like 'max_user_connections'");
        ...
        $out = 'max_connections (sunucu geneli limit): '.($maxConn[0]->Value ?? '?')."\n";
        $out .= 'max_user_connections (BU HESABA ozel limit, 0=sinirsiz/genel limite tabi): '.($maxUserConn[0]->Value ?? '?')."\n";
        $out .= 'Threads_connected (SU AN acik baglanti): '.($threadsConnected[0]->Value ?? '?')."\n";
        $out .= 'Max_used_connections (sunucu ac1l1s1ndan beri EN FAZLA ayni anda kullanilan): '.($maxUsedConnections[0]->Value ?? '?')."\n";
        $out .= 'Aborted_connects (basarisiz baglanti denemesi toplam): '.($aborted[0]->Value ?? '?')."\n";

        try {
            $processCount = DB::select('show processlist');
            $out .= 'Su anki processlist toplam oturum sayisi: '.count($processCount)."\n";
            $byUser = collect($processCount)->groupBy('User')->map->count();
            foreach ($byUser as $user => $count) {
                $out .= "  kullanici={$user}: {$count} baglanti\n";
            }
        } catch (\Throwable $e) {
            $out .= 'processlist okunamadi (yetki sinirli olabilir): '.$e->getMessage()."\n";
        }
    ...
    /**
     * 7 Eylul 2026: kullanicinin talebi - facility_users#293 (yer-tutucu)
     * temizlendikten sonra, kullanicinin "mevcut sifreyi gorebilir miyim"
     * sorusuna GERCEK cevap - sifreler hash'li, geri okunamaz. Bunun yerine
     * GERCEK Admin\UserController::resetFacilityUserPassword() ile ayni
     * (test degil, gercek) yolu kullanarak sifreyi sifirlar, yeni sifreyi
     * dogrudan gosterir.
     */
    private function facilityUserResetPassword(Request $incomingRequest): string
    {
        $admin = DB::table('admins')->first();
        if (! $admin) {
            return 'HATA: hic admin yok.';
        ...
        $response = $controller->resetFacilityUserPassword($facilityUser);

        return (string) ($response->getSession()?->get('success') ?? 'Islem tamamlandi ama mesaj okunamadi.');
    }

    /**
     * 7 Eylul 2026: kullanicinin canlida yasadigi kesinti (yanlislikla
     * tek-satira sikistirilmis @if/@else Blade sozdizimi hatasi) sirasinda
     * olusan ViewException kayitlarini toplu cozuldu isaretler - hepsi
     * duzeltmeden ONCEKI ayni olaydan, ayri ayri incelemeye gerek yok.
     */
    private function resolveOldErrorsBefore(Request $incomingRequest): string
    {
        $before = $incomingRequest->query('before', now()->toDateTimeString());
        $count = DB::table('platform_errors')
            ->whereNull('resolved_at')
            ->where('created_at', '<=', $before)
            ->update(['resolved_at' => now()]);

        return "Cozuldu olarak isaretlenen: {$count} kayit ({$before} tarihinden once olusanlar).";
    }
