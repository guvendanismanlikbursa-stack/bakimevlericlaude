...
<p class="text-sm text-gray-500 mb-4">Bu ayarlar 3 sitenin de ortak kullandığı banka hesap bilgisi, teklif ücreti ve fiyat segmentleridir.</p>
...
<form method="POST" action="{{ route('admin.settings.update') }}" class="bg-white rounded-xl shadow-sm p-6 grid md:grid-cols-2 gap-4 max-w-2xl">
  ...
  <div class="md:col-span-2">
    ...
    <input type="text" name="bank_name" value="{{ old('bank_name', $settings['bank_name']) }}" required class="border rounded-lg px-3 py-2 w-full mt-1">
  </div>
  <div class="md:col-span-2">
    <label class="text-sm font-medium">Hesap Sahibi</label>
    <input type="text" name="bank_account_holder" value="{{ old('bank_account_holder', $settings['bank_account_holder']) }}" required class="border rounded-lg px-3 py-2 w-full mt-1">
  </div>
  <div class="md:col-span-2">
    <label class="text-sm font-medium">IBAN</label>
    <input type="text" name="bank_iban" value="{{ old('bank_iban', $settings['bank_iban']) }}" required class="border rounded-lg px-3 py-2 w-full mt-1">
  </div>
  <div class="md:col-span-2">
