...
<form method="GET" data-instant-filter="1" data-results-target="js-facility-users-results" class="js-instant-filter bg-white rounded-xl shadow-sm border border-gray-100 p-4 grid md:grid-cols-4 gap-3 mb-6">
  ...
  <select name="category" onchange="this.form.submit()" class="border rounded-lg px-3 py-2 text-sm md:col-span-2"><option value="">Tüm kategoriler</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected((string) request('category')===(string) $category->id)>{{ $category->name }}</option>@endforeach</select>
