    ...
    <tbody class="divide-y">
      @forelse($reviews as $review)
        <tr>
          ...
          <td class="p-3"><span class="rounded-full px-2 py-1 text-xs font-bold {{ $review->status === 'approved' ? 'bg-green-100 text-green-700' : ($review->status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">{{ ['pending' => 'Bekliyor', 'approved' => 'Onaylı', 'rejected' => 'Reddedildi'][$review->status] ?? $review->status }}</span></td>
