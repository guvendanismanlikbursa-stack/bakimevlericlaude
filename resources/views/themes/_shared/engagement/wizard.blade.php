...
@endif
    ...
    <div class="{{ $variant === 'dark' ? 'bg-white text-gray-900 rounded-xl' : ($variant === 'warm' ? 'max-w-5xl mx-auto bg-white rounded-3xl' : 'bg-gray-50 rounded-xl') }} border border-gray-100 shadow-xl p-5 md:p-7">
      <form method="GET" action="{{ $resultHref }}" data-district-map='@json($districtMap)' class="js-location-filter grid md:grid-cols-2 gap-4">
        ...
        <label class="block"><span class="text-xs font-black text-gray-500">Bütçe</span><select name="price_max" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-3 bg-white"><option value="">Belirtmek istemiyorum</option><option value="10000">10.000 TL ve altı</option><option value="20000">20.000 TL ve altı</option><option value="30000">30.000 TL ve altı</option><option value="50000">50.000 TL ve altı</option></select></label>
