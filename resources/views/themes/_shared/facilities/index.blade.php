...
@else
    ...
    <form method="GET" data-district-map='@json($districtMap)' data-auto-submit="1" data-count-url="{{ brand_route('facilities.count') }}" class="js-location-filter bg-white rounded-xl shadow-sm p-4 grid sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-8 border border-gray-100">
      ...
      <select name="price_tier" aria-label="Fiyat segmenti" class="border rounded-lg px-3 py-2.5 text-sm bg-white"><option value="">Tüm segmentler</option>@foreach(['ekonomik' => '🟢 Ekonomik', 'standart' => '🔵 Standart', 'premium' => '🟣 Premium', 'ultra_premium' => '🟡 Ultra Premium'] as $value => $label)<option value="{{ $value }}" @selected(request('price_tier') === $value)>{{ $label }}</option>@endforeach</select>
