  ...
  <div class="relative max-w-6xl mx-auto px-4 py-12 lg:py-16">
    <div class="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
      ...
      <form method="GET" action="{{ brand_route('facilities.index') }}" data-district-map='@json($districtMap)' data-count-url="{{ brand_route('facilities.count') }}" class="js-location-filter bg-white text-gray-900 rounded-xl shadow-2xl p-5 border border-white/20">
        ...
        <div class="space-y-3">
          <select name="city" aria-label="İl" class="js-city w-full border border-gray-200 rounded-lg px-3 py-3 text-sm bg-white"><option value="">İl seçin</option>@foreach($cities as $city)<option value="{{ $city->slug }}">{{ $city->name }}</option>@endforeach</select>
          <select name="district" aria-label="İlçe" class="js-district w-full border border-gray-200 rounded-lg px-3 py-3 text-sm bg-white" disabled><option value="">Önce il seçin</option></select>
          <select name="category" aria-label="Kurum türü" class="w-full border border-gray-200 rounded-lg px-3 py-3 text-sm bg-white"><option value="">Kurum türü</option>@foreach($categories as $cat)<option value="{{ $cat->slug }}">{{ $cat->name }}</option>@endforeach</select>
          <select name="service" aria-label="Kurumun özellikleri" class="w-full border border-gray-200 rounded-lg px-3 py-3 text-sm bg-white"><option value="">Kurumun özellikleri</option>@foreach($sectionServices as $service)<option value="{{ $service }}">{{ $service }}</option>@endforeach</select>
          <select name="price_tier" aria-label="Fiyat segmenti" class="w-full border border-gray-200 rounded-lg px-3 py-3 text-sm bg-white"><option value="">Tüm segmentler</option>@foreach(['ekonomik' => '🟢 Ekonomik', 'standart' => '🔵 Standart', 'premium' => '🟣 Premium', 'ultra_premium' => '🟡 Ultra Premium'] as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select>
